...
router.get('/:userid', async (req, res) => {
    try {
        const allRecipes = await db.Recipe.find({user:req.params.userid})
        res.json(allRecipes)
    } catch (err) {
        console.log(err)
        res.status(503).json({
...
router.post('/:userid', async (req, res) => {
    ...
    foundUser.recipes.push(newRecipe.id)
        ...
        // handle error
           ...
           console.log(err)
           res.status(503).json({message: 'recipe not created'})
    }
})
...
router.put('/:id', async (req, res) => {
    try {
        const recipe = await db.Recipe.findByIdAndUpdate(req.params.id, req.body, {new:true}) 
        if (!recipe)
            return res.status(404).json({
                message: 'not atm'
            })
